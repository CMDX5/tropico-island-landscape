...
import { islandHeight, type Placement } from './terrain'

/* -------------------------------------------------------------------------- */
/*  Caribbean house                                                            */
/* -------------------------------------------------------------------------- */

// Bright tropical house palette (Tropico/Caribbean vibe)
 ...
 * a steep pyramidal terracotta roof and a tiny door + two windows.
 * Purely procedural low-poly, flat-shaded to match the toon terrain.
 */
...
/**
 * Place a small cluster of houses on a flat, low-altitude spot near the
 * beach. Candidates are drawn in a disc around `center` and kept only if
 * the terrain is low enough and the slope is gentle.
 */
function placeVillage(
  ...
  while (result.length < count && attempts < count * 40) {
    ...
    const r = 2 + rand() * 9 // loose cluster, 2–11 units from center
