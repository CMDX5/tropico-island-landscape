...
- **[Node.js](https://nodejs.org/) ≥ 20** (inclut `npm`). Next.js 16 nécessite Node 20+.
  - Vérifie ta version : `node --version`
  - Si tu ne l'as pas, télécharge la version **LTS** sur https://nodejs.org puis **ferme et rouvre ton terminal**.

> 💡 Bun / pnpm / yarn fonctionnent aussi, mais ce guide utilise **npm** (déjà inclus avec Node) pour rester universel.
...
Quand tu vois `✓ Ready in ...`, le serveur tourne.

> ⚠️ **Alternative** si `npm run dev` pose problème : `npx next dev -p 3000`
...
Ouvre simplement **http://localhost:3000** dans ton navigateur après avoir lancé `npm run dev`.
...
1. Le serveur doit tourner (`npm run dev`)
